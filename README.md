...
- [x] Confirm PokemonPriceTracker's terms explicitly allow commercial/public use before launch — **re-verified 2026-07-10** against the actual Terms of Service (`/terms`), not just the pricing page's feature table (whose "Commercial use license" row shows a red X for Free/API - misleading at a glance). The ToS itself states: "You are free to use the data for virtually any purpose, including... building applications, websites, bots, tools, or services that consume our data," with no tier distinction. The only restriction anywhere in the ToS is against reselling/redistributing their raw data as a standalone data product - not applicable here, since this site displays a computed view, not raw data resale. Best read: the pricing table's checkbox likely refers to that narrower data-resale/redistribution license (available as a paid add-on on Business+), not general public/commercial deployment. Not 100% certain without asking PokemonPriceTracker directly, but the ToS's own plain-language wording is the authoritative source and it's unambiguous on this point.
